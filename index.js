 ...
 * <ul>
 * <li> `browser:uncompressed` - build uncomprssed browser bundle (`ioncore-*.js`)
 * <li> `browser:compressed` - build compressed browser bundle (`ioncore-*.min.js`)
...
function startGulp(name, opts) {
  ...
  var fullname = name ? 'ioncore-' + name : 'ioncore';
  var files = ['lib/**/*.js'];
  var tests = ['test/**/*.js'];
  var alljs = files.concat(tests);

  var buildPath = './node_modules/ioncore-build/';
  ...
  if (browser) {
    ...
    if (name !== 'lib') {
      browserifyCommand = browserifyPath + ' --require ./index.js:' + fullname + ' --external ioncore-lib -o ' + fullname + '.js';
    } else {
      browserifyCommand = browserifyPath + ' --require ./index.js:ioncore-lib -o ioncore-lib.js';
    }
